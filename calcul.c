...
Matrice mom(BmpImg* pic, int n) {
	 ...
	 * a chaque ligne i, le nombre de colonnes sera N - i
	 * On obtiendra alors une matrice triangulaire inversé
	 */
	for(int p = 0; p <= n; p++) {
		for(int q = 0; q <= n-p; q++) {
			mat.tab[p][q] = 0;				// Initialisation a 0, je fais pas confiance au calloc;
...
double base_legendre(double x, int n, Matrice coef){
    double res=0;
    for(int i=0; i<=n; i++){
        res+=(coef.tab[n][i])*pow(x,i);					// somme des termes du polyneme
    }
    return res;
}
...
Matrice coefLegendre(int n) {
	...
	for(int i=2; i<=n; i++){    //Determination de tous les an jusqu'a l'ordre souhaite
...
Matrice mom_legendre(BmpImg* pic, int n) {
	...
	Matrice eta = momcentre(pic, n);         //Recuperation des moments centres et normes
	double Cpq = 0;			         //coefficients de normalisation initialises a 0
	Matrice coef = coefLegendre(n);
	for(int p = 0; p <= n; p++) {
		for(int q = 0; q <= n-p; q++){
			lambda.tab[p][q] = 0;	 	//Initialisation a 0 (en plus du calloc)
	...
	supprMatrice(&coef);		// desallocation de la matrice des coefficients
	supprMatrice(&eta);		// desallocation de la matrice des moments centres normes
	return lambda;
}
...
double distance_euclidienne(Matrice mat1, Matrice mat2, int ordre){
    ...
    if (mat1.N < ordre || mat2.N < ordre){ 				// si une des matrices a un ordre inferieur a l'ordre de comparaison
        printf("ERREUR : Les matrices sont trop petites\n");
        return 0;
    }else{								// dans le cas contraire...
        for(p = 0 ; p < ordre; p++){
            for(q = 0 ; q < ordre - p;q++){
                res += pow(mat1.tab[p][q]-mat2.tab[p][q],2);		// somme des differences des moments au carre
            }
        }
    }
